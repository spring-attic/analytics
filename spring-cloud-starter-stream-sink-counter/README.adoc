...
$$counter.tag.fields$$:: $$The field names to extract tag values for the counter increment. Single field can produce an array of values, which in turn means distinct name/value tags. Every name/value tag causes a separate counter increment.$$ *($$List<String>$$, default: `$$<none>$$`)*
